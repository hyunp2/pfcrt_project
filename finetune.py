...
#             print(cf.red(f"0: {valid_targets0.size()}, 1: {valid_targets1.size()}, 2: {valid_targets2.size()}" ))
    ...
    def loss(self, predictions: dict, targets: torch.Tensor) -> torch.tensor:
        ...
        if not self.hparam.finetune:
            ...
            losses = w0 * loss0_fn(predictions["logits0"], targets["labels"][:,0].long() if not self.hparam.use_ce else targets["labels"][:,0].to(torch.float32)) + w1 * loss1_fn(predictions["logits1"], targets["labels"][:,1].long() if not self.hparam.use_ce else targets["labels"][:,1].to(torch.float32)) + w2 * loss2_fn(predictions["logits2"], targets["labels"][:,2].long() if not self.hparam.use_ce else targets["labels"][:,2].to(torch.float32))
            return losses.mean()
